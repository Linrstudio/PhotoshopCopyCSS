...
*/
/*
	Modified by Linr 2018.2.8 v2.0.0 www@xiaogezi.cn
*/
//// Linr
...
PSLayerInfo.prototype.descToCSSColor = function( colorDesc, colorPath )
{
	...
	if (! rgbTxt)
		return null;
	//return "rgb(" + rgbTxt.join(", ") + ")";	////cmt by Linr
	return rgb2hex("rgb(" + rgbTxt.join(",") + ")"); //Linr
}
...
PSLayerInfo.prototype.descToRGBAColor = function( colorPath, opacity, colorDesc )
{
	...
	if (opacity == 1.0)
		...
		//return "rgba(" + rgbTxt.join(", ") + ", " + round1k( opacity ) + ")"; ////cmt by Linr
		return "rgba(" + rgbTxt.join(",") + "," + round1k( opacity ) + ")"; //Linr
}
...
cssToClip.addObjectBounds = function( boundsInfo )
{
	...
	if (! boundsInfo.textLine)
	{
		...
		// Linr
		isIco && this.addText( "vertical-align: middle;");
	...
	if (! boundsInfo.textLine)
	{
		...
		// Linr
		var isIcon = isIcon(width, height);
		if(isIcon){
			this.addText( "display: inline-block;");
		}
		...
		// Linr
		isIcon && this.addText( "vertical-align: middle;");
...
cssToClip.getPixelLayerCSS = function()
{
	...
	else
	{
		...
		this.addText( 'background-image: url(' + docFolder + name + ');');	// Linr remove "
	}
	var fillOpacity = this.getLayerAttr("fillOpacity")/255.0;
